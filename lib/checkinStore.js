...
function createCheckinStore(pool, options = {}) {
  ...
  return {
    ...
    async deleteAccounts(addresses) {
      await initialize();
      const keys = addresses.map(address => `account:${address}`);
      const result = await pool.query(`DELETE FROM ${schema}.records WHERE key = ANY($1::text[])`, [keys]);
