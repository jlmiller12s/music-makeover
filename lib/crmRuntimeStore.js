...
function getPostgresPool(connectionString) {
  if (!globalThis[POSTGRES_POOL_KEY]) {
    ...
    globalThis[POSTGRES_POOL_KEY] = new Pool(createPostgresPoolOptions(connectionString));
  }
  return globalThis[POSTGRES_POOL_KEY];
}

function createPostgresPoolOptions(connectionString) {
  const poolOptions = {
    connectionString: normalizePostgresConnectionString(connectionString),
  };
  if (requiresSslRelaxation(connectionString)) {
...
function normalizePostgresConnectionString(connectionString) {
  if (!requiresSslRelaxation(connectionString)) return connectionString;
...
function requiresSslRelaxation(connectionString) {
