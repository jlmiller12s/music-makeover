...
function createCheckinService({ store, sendEmail, emailReady, now = Date.now }) {
  ...
  return { begin, requestCode, verifyCode, participant, logout, saveDraft, submit, saveFeedback, allow, revoke, deleteParticipants, resetPreview, list, detail, review, notify };
}
function publicSubmission(submission) {
  if (!submission) return null;
  return { id: submission.id, submittedAt: submission.submittedAt, reviewStatus: submission.review.status, snapshot: submission.snapshot, feedback: submission.feedback || null };
}
