...
function createCheckinService({ store, sendEmail, emailReady, now = Date.now }) {
  ...
  async function saveClarity(token, value) {
    if (!Number.isInteger(value) || value < 1 || value > 5) throw fail('Please select a clarity response from 1 to 5.');
    const { address, digest } = sessionParts(token);
    return store.mutate(`account:${address}`, a => {
      if (!authorized(a, digest)) throw fail('Please sign in to continue.', 401);
      if (!a.submission) throw fail('Complete your assessment before sharing this reflection.', 409);
      a.submission.postClarity = { value, savedAt: new Date(now()).toISOString() };
      return a.submission.postClarity;
    });
  }
  async function saveFeedback(token, input) {
    if (!input || typeof input !== 'object' || Array.isArray(input)) throw fail('Please check your feedback.');
    const answers = {};
    for (const key of ['feltTrue', 'missing', 'unclear', 'clearer']) {
      const value = input[key] ?? '';
      if (typeof value !== 'string' || value.length > 3000) throw fail('Please keep each feedback response under 3,000 characters.');
      if (!value.trim()) throw fail('Please respond to all four feedback questions. If nothing comes to mind, you can write “Nothing comes to mind”.');
      answers[key] = value;
    }
    const { address, digest } = sessionParts(token);
    return store.mutate(`account:${address}`, a => {
      if (!authorized(a, digest)) throw fail('Please sign in to continue.', 401);
      if (!a.submission) throw fail('Complete your assessment before sharing feedback.', 409);
      a.submission.feedback = { answers, savedAt: new Date(now()).toISOString() };
      return a.submission.feedback;
    });
  }
  ...
  return { begin, requestCode, verifyCode, participant, logout, saveDraft, submit, saveClarity, saveFeedback, allow, revoke, deleteParticipants, resetPreview, list, detail, review, notify };
}
function publicSubmission(submission) {
  if (!submission) return null;
  return { id: submission.id, submittedAt: submission.submittedAt, reviewStatus: submission.review.status, snapshot: submission.snapshot, postClarity: submission.postClarity || null, feedback: submission.feedback || null };
}
