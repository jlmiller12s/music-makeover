...
function createAdmin(authState, payload, options = {}) {
  ...
  const setupCode = options.setupCode || process.env.ADMIN_SETUP_CODE || (isProduction() ? '' : 'musicmakeover2026');
  ...
  if (isProduction() && state.admins.length > 0 && !hasAdminSession) {
    throw new Error('An active admin session is required to create another admin');
  }
...
function verifyAdminSession(authState, token, options = {}) {
  ...
  // Production sessions must remain revocable after password reset or removal.
  const signedAdmin = !isProduction() && verifySignedSessionToken(token, { secret: options.sessionSecret });
...
function verifySignedSessionToken(token, options = {}) {
  ...
  try {
    ...
    if (!Number.isFinite(new Date(payload.exp).getTime()) || new Date(payload.exp) <= new Date()) return null;
...
function getSessionSecret(secret) {
  // A per-process key is safe for state-backed sessions if no secret is configured.
  // Never use a public development secret on a hosted deployment.
  return String(
    secret
    || process.env.ADMIN_SESSION_SECRET
    || process.env.AUTH_SECRET
    || process.env.ADMIN_SETUP_CODE
    || (isProduction() ? productionSessionSecret : 'music-makeover-dev-session-secret'),
  );
}

const productionSessionSecret = crypto.randomBytes(32).toString('hex');
function isProduction() { return process.env.NODE_ENV === 'production' || Boolean(process.env.VERCEL); }
