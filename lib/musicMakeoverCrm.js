...
const DEFAULT_SITE_CONTENT = {
  ...
  contact: {
    ...
    tiktokUrl: 'https://www.tiktok.com/@theofficialmusicmakeover?is_from_webapp=1&sender_device=pc',
    linkedinUrl: 'https://www.linkedin.com/in/ashley-miller-themusicmakeover/',
  },
  testimonials: {
...
function createDefaultState(nowInput = new Date().toISOString(), includeSeed = false) {
  ...
  return {
    ...
    inquiries: includeSeed ? seedInquiries(now) : [],
    clients: includeSeed ? seedClients(now) : [],
    appointments: includeSeed ? seedAppointments(now) : [],
    proposals: includeSeed ? seedProposals(now) : [],
    contracts: includeSeed ? seedContracts(now) : [],
    invoices: includeSeed ? seedInvoices(now) : [],
    payments: includeSeed ? seedPayments(now) : [],
    tasks: includeSeed ? seedTasks(now) : [],
    notes: includeSeed ? seedNotes(now) : [],
    resources: includeSeed ? seedResources(now) : [],
    testimonials: includeSeed ? seedTestimonials(now) : [],
    mediaReleases: includeSeed ? seedMediaReleases(now) : [],
    consultationNotes: includeSeed ? seedConsultationNotes(now) : [],
    aiRecommendations: [],
    inquiryEmailOutbox: [],
...
function normalizeCrmState(state, nowInput = new Date().toISOString()) {
  ...
  // Filter out any seed records from existing state
  if (Array.isArray(nextState.inquiries)) {
    nextState.inquiries = nextState.inquiries.filter((inquiry) => !String(inquiry.id).includes('seed'));
  }
  if (Array.isArray(nextState.clients)) {
    nextState.clients = nextState.clients.filter((client) => !String(client.id).includes('seed'));
  }
  if (Array.isArray(nextState.appointments)) {
    nextState.appointments = nextState.appointments.filter((appointment) => !String(appointment.id).includes('seed'));
  }
  if (Array.isArray(nextState.proposals)) {
    nextState.proposals = nextState.proposals.filter((proposal) => !String(proposal.id).includes('seed'));
  }
  if (Array.isArray(nextState.contracts)) {
    nextState.contracts = nextState.contracts.filter((contract) => !String(contract.id).includes('seed'));
  }
  if (Array.isArray(nextState.invoices)) {
    nextState.invoices = nextState.invoices.filter((invoice) => !String(invoice.id).includes('seed'));
  }
  if (Array.isArray(nextState.payments)) {
    nextState.payments = nextState.payments.filter((payment) => !String(payment.id).includes('seed'));
  }
  if (Array.isArray(nextState.tasks)) {
    nextState.tasks = nextState.tasks.filter((task) => !String(task.id).includes('seed') && !/^task_[1-7]$/.test(task.id));
  }
  if (Array.isArray(nextState.notes)) {
    nextState.notes = nextState.notes.filter((note) => !String(note.id).includes('seed'));
  }
  if (Array.isArray(nextState.resources)) {
    nextState.resources = nextState.resources.filter((resource) => !String(resource.id).includes('seed'));
  }
  if (Array.isArray(nextState.testimonials)) {
    nextState.testimonials = nextState.testimonials.filter((testimonial) => !String(testimonial.id).includes('seed'));
  }
  if (Array.isArray(nextState.mediaReleases)) {
    nextState.mediaReleases = nextState.mediaReleases.filter((release) => !String(release.id).includes('seed'));
  }
  if (Array.isArray(nextState.consultationNotes)) {
    nextState.consultationNotes = nextState.consultationNotes.filter((note) => !String(note.id).includes('seed'));
  }
...
function normalizeSiteContent(siteContent, nowInput = new Date().toISOString()) {
  ...
  if (normalized.contact) {
    ...
    normalized.contact.tiktokUrl = 'https://www.tiktok.com/@theofficialmusicmakeover?is_from_webapp=1&sender_device=pc';
  }
...
function normalizeTestimonial(input, nowInput = new Date().toISOString()) {
  ...
  const isGuest = normalized.source === 'guest-testimonial-form' || normalized.isGuest === 'true';
  if (isGuest) {
    if (!normalized.name) normalized.name = 'Guest';
    if (!normalized.serviceType) normalized.serviceType = 'Other';
    if (!normalized.challengeGoal) normalized.challengeGoal = 'N/A (Guest Submission)';
    if (!normalized.standout) normalized.standout = 'N/A (Guest Submission)';
    if (!normalized.changedImproved) normalized.changedImproved = 'N/A (Guest Submission)';
    if (!normalized.sharePermission) normalized.sharePermission = 'yes';
    if (!normalized.nameDisplay) normalized.nameDisplay = 'Anonymous';
  }
  ...
  return {
    ...
    fullTestimonial: isGuest
      ? `Guest Testimonial:\n\n${normalized.considerationQuote}`
      : [
...
function buildTestimonialAdminNotificationEmail(testimonial, nowInput = new Date().toISOString()) {
  const isGuest = testimonial.source === 'guest-testimonial-form';
  const bodyParts = [
  ...
  return {
    ...
    subject: `New ${isGuest ? 'guest ' : ''}testimonial from ${testimonial.clientName}`,
    body: bodyParts.join('\n'),
...
function clearState(state, nowInput = new Date().toISOString()) {
  const now = new Date(nowInput);
  return {
    ...state,
    generatedAt: now.toISOString(),
...
function updateTestimonialStatus(state, testimonialId, status, nowInput = new Date().toISOString()) {
  const allowedStatuses = ['submitted', 'approved', 'denied', 'requested', 'draft'];
  if (!allowedStatuses.includes(status)) {
    throw new Error(`Unknown testimonial status: ${status}`);
  }
  const nextState = clone(state);
  let found = false;
  nextState.testimonials = (nextState.testimonials || []).map((item) => {
    if (item.id !== testimonialId) return item;
    found = true;
    const approvedForWebsite = status === 'approved';
    return {
      ...item,
      status,
      approvedForWebsite,
  ...
  if (!found) throw new Error(`Testimonial not found: ${testimonialId}`);
  return nextState;
}

function deleteTestimonials(state, testimonialIds, nowInput = new Date().toISOString()) {
  if (!Array.isArray(testimonialIds)) {
    throw new Error('testimonialIds must be an array');
  }
  const nextState = clone(state);
  nextState.testimonials = (nextState.testimonials || []).filter(
    (item) => !testimonialIds.includes(item.id)
  );
  nextState.generatedAt = new Date(nowInput).toISOString();
  return nextState;
}
