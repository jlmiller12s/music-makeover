...
const outlookLabels = ['Significant sustainability concern', 'Probably not sustainable', 'Unsure', 'Mostly sustainable', 'Very sustainable'];
...
function validateInput(input, complete = true) {
  ...
  for (const field of ['heaviest', 'hope']) {
    ...
    if (complete && !value.trim()) throw new Error('Please respond to both opening reflections.');
    result[field] = value;
  }
